...
export const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ label, id, name, type = 'text', error, icon, className = '', wrapperClassName = '', ...props }, ref) => {
    ...
    const baseInputStyle = "block w-full rounded-md bg-jdc-gray-800 border-transparent focus:border-jdc-yellow focus:ring focus:ring-jdc-yellow focus:ring-opacity-50 placeholder-jdc-gray-400 text-white";
        ...
        <div className="relative rounded-md shadow-sm">
          {hasIcon && (
            <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
              {React.cloneElement(icon, { className: 'h-5 w-5 text-jdc-gray-400' })}
            </div>
          )}
          <input
            ref={ref}
